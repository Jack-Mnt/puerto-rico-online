...
export function ModeradorHistorial() {
  ...
  const filtered = useMemo(() => {
    return data.filter((p) => {
      ...
      if (sedeFiltro && p.sede_id !== sedeFiltro) return false;
  ...
  }, [data, estado, sedeFiltro, fecha, tipoEntrega]);
    ...
    <div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-2 mb-4">
        <select className="input" value={estado} onChange={(e) => setEstado(e.target.value as any)}>
          ...
          <option value="pedido_entregado">Entregado</option>
          <option value="pedido_cancelado">Cancelado</option>
        </select>
        <select className="input" value={sedeFiltro} onChange={(e) => setSedeFiltro(e.target.value)}>
      ...
      {isLoading ? (
        ...
        <EmptyState title="Sin pedidos en el historial" />
      ) : (
            ...
            {filtered.map((p) => {
              ...
              return (
                ...
                >
                  ...
                  <div className="md:col-span-2 truncate text-muted-foreground">{sedeNombre}</div>
                  <div className="md:col-span-1">
                    <span
                      className={`inline-block rounded-full border px-2 py-0.5 text-[10px] ${ESTADO_COLOR[p.estado]}`}
                    >
            ...
            <div className="space-y-2">
              {items?.map((d) => (
                ...
                >
                  {d.productos?.imagen ? (
                    <img
                      src={storageUrl(d.productos.imagen)}
                      alt=""
